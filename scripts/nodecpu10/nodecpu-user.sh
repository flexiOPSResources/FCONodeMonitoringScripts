...
CLUSTER1=/opt/extility/skyline/war/nodecpuuser/
CLUSTER2=/opt/extility/skyline/war/nodecpuuser/
...
getdatacluster1 ()
        ...
        {
                #Get data
	sshe $i "/usr/bin/mpstat -P ALL | /usr/bin/mpstat -P ALL | /usr/bin/awk -v DATE=$(date '+%Y-%m-%dT%H:%M:%S') 'NR>4 {print DATE\",\"\$2\",\"\$3}'" >> $CLUSTER1$i.csv
        }
...
getdatacluster2 ()
	{
                #GET data
                sshe 11.222.33.44 "/root/scripts/nodecpu/getusage-user.sh $n " >> $CLUSTER2$n.csv ;
	}
